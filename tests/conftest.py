...
def lista_eventos_normais():
    ...
    for i in range(100):
        eventos.append({
            ...
            "count":          int(np.random.normal(10, 3).clip(1, 30)),
            "bytes_sent":     int(np.random.exponential(1000).clip(0, 50_000)),
            "bytes_received": int(np.random.exponential(5000).clip(0, 100_000)),
            "duration_ms":    float(np.random.exponential(200).clip(0, 5_000)),
